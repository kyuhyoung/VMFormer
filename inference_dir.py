        ...
        for name_file_dir in os.listdir(direc):
            ...
            if os.path.isfile(path_file_dir):
                if is_extension_given:
                    ...
                    li_path_total.append(path_file_dir)
    return natsorted(li_path_total)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        files = get_list_of_file_path_under_1st_with_3rd_extension(args.img_path, False, args.ext)
        ...
        #for single in sorted(os.listdir(clip_path)):
        ...
        for kk in range(len(files)):
            ...
            img_index_set.append(kk)
            if (kk+1) % args.num_frames == 0:
                ...
                for jj, mask in enumerate(outputs):
                    print(f'mask.shape b4 : {mask.shape}');#    exit(0)
                    mask = F.interpolate(mask, (img.shape[-2], img.shape[-1]), mode="bilinear", align_corners=False)
                    print(f'mask.shape after : {mask.shape}');#    exit(0)
                    pred_pha = mask[0][0].sigmoid().cpu().detach().numpy().astype(np.float32)
                    pred_pha = pred_pha[:, :, np.newaxis]
                   
                    # use numpy to convert the pil_image into a numpy array
                    numpy_image=np.array(li_im[jj])
                    # convert to a openCV2 image and convert from RGB to BGR format
                    im_ori_bgr = cv2.cvtColor(numpy_image, cv2.COLOR_RGB2BGR)
